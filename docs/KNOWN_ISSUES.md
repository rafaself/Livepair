...
- Screen context is speech-session-only. The shipped policy now supports manual explicit sends plus continuous 3000 ms base sends with bounded 1000 ms bursts, but HD screenshot tooling and broader tuning are still planned work.
...
- Do not describe screen context with removed pre-Wave-5 terminology. The current product model is Share Screen on/off plus a persisted `manual` or `continuous` send policy during an active speech session.
