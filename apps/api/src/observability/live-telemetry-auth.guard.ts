...
@Injectable()
export class LiveTelemetryAuthGuard implements CanActivate {
  canActivate(context: ExecutionContext): boolean {
    const request = context.switchToHttp().getRequest<RequestLike>();
    const requestContext = resolveSessionTokenRequestContext(request);

    if (!env.sessionTokenAuthSecret) {
      console.error('[observability:live-telemetry] auth unavailable', {
        ...requestContext,
        reason: 'auth_secret_missing',
      });
      throw new ServiceUnavailableException('Live telemetry auth secret is not configured');
    }

    const credential = readHeader(request, SESSION_TOKEN_AUTH_HEADER_NAME);

    if (typeof credential !== 'string' || credential.trim().length === 0) {
      console.warn('[observability:live-telemetry] auth rejected', {
        ...requestContext,
