variable "project_id" {
  description = "Google Cloud project ID for the current environment."
  type        = string
}

variable "region" {
  description = "Primary Google Cloud region for the current environment."
  type        = string
}
...
variable "api_migration_job" {
  description = "Cloud Run job settings for API database migrations."
  type = object({
    image               = string
    deletion_protection = optional(bool, false)
...
variable "api_secret_versions" {
  description = "Secret Manager versions to reference from Cloud Run. Keep latest for simple rotation or pin explicit versions later."
  type = object({
