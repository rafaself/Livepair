...
locals {
  ...
  service_account_configs = {
    ...
    api_migrator = {
      ...
      description   = "Runtime identity for the Livepair database migration job."
      project_roles = ["roles/cloudsql.client"]
    }
  }
...
module "cloud_run_job" {
  ...
  max_retries           = var.api_migration_job.max_retries
  cpu                   = var.api_migration_job.cpu
  memory                = var.api_migration_job.memory
  deletion_protection   = var.api_migration_job.deletion_protection
...
module "monitoring" {
  ...
  telemetry_metric_name_prefix               = "${replace(var.environment_name, "-", "_")}_"
